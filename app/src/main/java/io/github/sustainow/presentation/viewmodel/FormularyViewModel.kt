...
class FormularyViewModel
    ...
    constructor(
        ...
        private val calculationService: ConsumptionCalculationService,
        ...
        private val _totalConsumption = MutableStateFlow(TotalConsumption(0f, ""))
        val totalConsumption = _totalConsumption.asStateFlow()
        ...
        fun calculateTotalValue(unit: String)  {
            viewModelScope.launch {
                _loading.value = true
                try {
                    _totalConsumption.value = calculationService.calculateTotal(_currentAnswers.value, unit)
                } catch (e: Exception) {
                    _error.value = DataError(source = "answers", operation = DataOperation.GET)
                } finally {
                    _loading.value = false
                }
            }
